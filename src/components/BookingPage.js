...
import FilterCheckboxGroup from './FilterCheckboxGroup'; // Correct import
...
const BookingPage = () => {
  ...
  const [carData, setCarData] = useState([]); // State to hold car data fetched from the backend
  ...
  useEffect(() => {
    ...
    const fetchCarData = async () => {
      try {
        const response = await fetch('http://localhost:4000/api/cars'); // Replace with the actual API route
        if (!response.ok) {
          throw new Error('Failed to fetch car data');
        }
        const data = await response.json();
        setCarData(data); // Set the fetched car data to state
        setLoading(false);
      } catch (err) {
  ...
  // Filter the car data based on the filters state
